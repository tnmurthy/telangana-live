    ...
    return (
        <section className="animate-fade-in">
            <div className="section-header">
                <div>
                    <h2 className="section-title flex items-center gap-2">🚇 Public Transport</h2>
                    <p className="section-subtitle">Metro & MMTS 2026 updates</p>
                </div>
            </div>

            {/* Metro Takeover Banner */}
            <div className="glass-card p-4 mb-4 border-blue-500/15 flex items-center gap-3">
            ...
            <div className="glass-card section-block">
                <h3 className="label-xs mb-3">🚂 MMTS Schedule</h3>
                <div className="space-y-3 mb-4">
                    {mmtsData.routes.map((route) => (
                        <div key={route.name} className={`detail-box ${route.highlight ? 'border-heritage-gold/20' : ''}`}>
                            ...
                            {route.highlightNote && (
                                <p className="text-xs text-heritage-gold mt-2 font-medium">{route.highlightNote}</p>
                            )}
                        </div>
                    ))}
                </div>
                ...
                <div className="detail-box border-deep-pink/20 bg-deep-pink/[0.03]">
                    <div className="flex items-center gap-2 mb-2">
                        <span className="text-lg">{mmtsData.ladiesSpecial.icon}</span>
